...
void push(struct Stack* s, struct node* n) { if (s->top < s->cap-1) s->arr[++s->top] = n; }
struct node* pop(struct Stack* s) { return isEmpty(s) ? NULL : s->arr[s->top--]; }

// Zigzag Traversal (prints in list-of-lists format)
void zigzagTraversal(struct node* root) {
    ...
    while (!isEmpty(curr)) {
        ...
        while (!isEmpty(curr)) {
            ...
            if (temp) {
                if (firstPrinted) printf(",");  // comma between values
                printf("%d", temp->data);
                firstPrinted = 1;
        ...
        if (!isEmpty(next)) printf(", "); // comma between levels
